...
export const VerSolicitudVacaciones = () => {
  ...
  useEffect(() => {
    const fetchSolicitud = async () => {
      try {
        const response = await axios.get(`/obtenerSolicitudVacaciones/${idSolVac}`, { withCredentials: true });
  ...
  const handleConfirm2 = async () => {
    try {
      ...
      const responseUpdate = await axios.put('/cambiarEstadosolicitudVacacionesAprobada', {
        id: idSolVac,
      ...
      if (responseUpdate && selectedEstado2 === 'APROBADO') {
        ...
        const responseInsert = await axios.post('/createSolicitudAprobadaGerencia', {
          id: idSolVac,
          estado: selectedEstado2         
        ...
        if (responseInsert.status === 200) {
          message.success('Estado actualizado e inserción realizada correctamente');
      ...
      message.error({ content: 'Hubo un error al actualizar el estado o realizar la inserción' });
      setIsModalVisible2(false);
    }
  };
...
<div>
        <Row gutter={16}>
          <Col span={12}>
            <Statistic title="Saldo de dias de vacaciones" value={saldoVacaciones || 0} />
          </Col>         
        </Row>
      </div> 
      ...
      <Descriptions
        ...
        bordered
      >
        <Descriptions.Item label="Empleado">{solicitud.usrInsert}</Descriptions.Item>
        ...
        <Descriptions.Item label="Estado">{renderEstadoTag(solicitud.estadoVacaciones || solicitud.estado)}</Descriptions.Item>
      ...
      <Modal
        title="Modificar Estado de Solicitud"
